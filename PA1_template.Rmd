...
2, Devise a strategy for filling in all of the missing values in the dataset. I will use the mean of steps per day for replacing NAs. Some days have no steps data (all steps data are NA), so if it is, 0 (zero) is intoroduced.
...
```
The "steps_rep" is the new steps data filled in all the missing values.
...
-> There is one peak in the histogram from the first part. This histogram suggests the data have taken from one population.

On the other hand, there are two peaks in the histogram from this part.

The second histogram suggests that the data contains at least 2 population which have different features from each other. 
